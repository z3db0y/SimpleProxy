...
export default class Proxy {
    ...
    constructor(port: number, host = '0.0.0.0', authCallback: (username: string, password: string) => boolean = () => true, requestCallback: (request: Result) => Result = (req) => {
        return {
            isRequest: false,
            isResponse: true,
            headers: {},
    ...
    async onData(socket: Socket, target: Socket | undefined, data: Buffer) {
        ...
        if(request && request.isRequest && !target) {
            ...
            let username = '';
            let password = '';
            if(auth && !Array.isArray(auth) && auth.toLowerCase().startsWith('basic ')) {
                [username, password] = Buffer.from(auth.split(' ').slice(1).join(' '), 'base64').toString().split(':');
            }

            target = new Socket();

            let urlString = 'http' + (request.method == 'CONNECT' ? 's' : '') + '://';
